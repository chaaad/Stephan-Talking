	...
	process: function(text) {
		...
		function translate(str) {
			...
			do {
				...
				if (t_word_str.length>1) number_words_str= t_word_str +(t_ct ? $TT.fancyNumbers.map["t" +t_ct] : "") +number_words_str;
				t_ct++;
				str= str.slice(0, -3);
			...
			function translate_triplet(triplet_str) {
				...
				if (x>99) {
					...
					t_word_str+= $TT.fancyNumbers.map[ltr] +" " +$TT.fancyNumbers.map["100"] +" ";
				}
				...
				if (x) {
					if (x>9) {
						if (temp_str= $TT.fancyNumbers.map[ltr]) {
							t_word_str+= temp_str +" ";
							if (x<21) return;

						} else {
							ltr= triplet_str.slice(-2, -1);
							if (temp_str= $TT.fancyNumbers.map[ltr +"0"]) t_word_str+= temp_str +" ";
						}
					}

					ltr= triplet_str.slice(-1); //last char
					x= ltr *1;
					if (x) t_word_str+= $TT.fancyNumbers.map[ltr] +" ";
				}
			}
		}
